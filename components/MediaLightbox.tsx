...
export default function MediaLightbox({
  ...
  return (
    <div onClick={onClose}
      ...
      {/* Top bar */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', color: '#fff' }} onClick={e => e.stopPropagation()}>
        <span style={{ fontSize: 13, fontWeight: 600, opacity: 0.9, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{cur.name || ''}<span style={{ opacity: 0.6, marginLeft: 8 }}>{index + 1} / {n}</span></span>
