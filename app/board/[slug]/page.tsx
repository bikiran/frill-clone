...
export default function BoardPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--canvas)' }}>
      ...
      {/* Powered by Colvy — hidden if company has branding removal (business+ or $5 addon) */}
      {!(company?.remove_branding || company?.plan === 'business' || company?.plan === 'growth' || company?.plan === 'enterprise' || company?.plan === 'pro') && (
        <div className="text-center py-6 border-t mt-8" style={{ borderColor: 'var(--border)' }}>
          <a
            href={`https://colvy.com?ref=${company?.slug || ''}&utm_source=board&utm_medium=powered_by&utm_campaign=${company?.slug || ''}`}
            className="text-xs"
            style={{ color: 'var(--slate)' }}
            target="_blank"
            rel="noopener">
