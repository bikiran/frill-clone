      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, background: navBg, backdropFilter: navScrolled ? 'blur(18px)' : 'none', borderBottom: `1px solid ${navScrolled ? cardBorder : 'transparent'}`, transition: 'all 0.3s' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 24px', height: 68, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div className="fx-desktop" style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            {[{ label: 'Inbox & CRM', href: '/inbox-crm' }, { label: 'Ideas', href: '/features/ideas' }, { label: 'Roadmap', href: '/features/roadmap' }, { label: 'Announcements', href: '/features/announcements' }, { label: 'Pricing', href: '/pricing' }].map((n) => (<a key={n.label} href={n.href} className="fx-navlink" style={{ padding: '8px 14px', borderRadius: 10, fontSize: 14.5, fontWeight: 600, color: muted, textDecoration: 'none' }}>{n.label}</a>))}
          </div>
            ...
            return (
              <Reveal key={f.title} delay={(i % 2) * 0.06}>
                <div className="fx-card" style={{ padding: 30, borderRadius: 22, background: cardBg, border: `1px solid ${cardBorder}`, height: '100%' }}>
                  <div style={{ width: 54, height: 54, borderRadius: 16, background: c + '18', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, marginBottom: 16 }}>{f.icon}</div>
