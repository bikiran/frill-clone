...
export default function FeatureDetailPage() {
  ...
  const mockKind = (FT[slug] ? slug : 'inbox') as MockKind
  ...
  return (
    <div style={{ background: bg, color: text, fontFamily: font, minHeight: '100vh', overflowX: 'hidden', transition: 'background 0.3s, color 0.3s' }}>
      ...
      <section className="fs-hero" style={{ position: 'relative', display: 'grid', gridTemplateColumns: '1.05fr 0.95fr', gap: 48, alignItems: 'center', maxWidth: 1280, margin: '0 auto', padding: '150px 24px 60px' }}>
        ...
        <Reveal delay={0.1}>
          <div style={{ position: 'relative', borderRadius: 28, minHeight: 340, padding: 26, display: 'flex', alignItems: 'center', justifyContent: 'center', background: `linear-gradient(150deg, ${accent} 0%, ${accent}cc 48%, ${dark ? '#0b0c14' : '#171a2b'} 125%)`, boxShadow: `0 30px 70px ${accent}44` }}>
            {/* Designed product-UI mockup for this feature */}
            <div style={{ width: '100%', maxWidth: 430, animation: 'fsFloat 7s ease-in-out infinite' }}>
              <FeatureMockup kind={mockKind} accent={accent} dark={false} />
            </div>
            {/* Floating labels for flair */}
            {f.heroChips.slice(0, 2).map((c, i) => (
              <span key={c} style={{ position: 'absolute', ...(i === 0 ? { top: 16, left: 16 } : { bottom: 16, right: 16 }), fontSize: 12.5, fontWeight: 800, color: '#fff', background: 'rgba(255,255,255,0.2)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.36)', borderRadius: 999, padding: '7px 14px', animation: `fsFloat ${5 + i * 0.7}s ease-in-out ${i * 0.3}s infinite` }}>{c}</span>
            ))}
          </div>
        </Reveal>
      </section>
