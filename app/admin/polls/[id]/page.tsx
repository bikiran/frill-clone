    ...
    <div className="max-w-3xl mx-auto px-4 md:px-6 py-8">
      ...
      <div className="bg-white rounded-2xl border p-6 mb-6" style={{ borderColor: 'var(--border)' }}>
        ...
        {(!poll.options || poll.options.length === 0) ? (
          <div className="p-4 rounded-lg text-center" style={{ background: '#fef2f2', color: '#dc2626' }}>
            <p className="text-sm font-medium">No options found for this poll.</p>
            <p className="text-xs mt-1">Edit the poll to add answer options.</p>
          </div>
        ) : (
            ...
            return (
              <div key={opt}>
                <div className="flex justify-between text-sm mb-1.5">
                  ...
                  <span className="font-semibold" style={{ color: 'var(--coral)' }}>{count} ({pct}%)</span>
                </div>
                <div className="h-3 rounded-full bg-gray-100">
                  <div className="h-3 rounded-full transition-all duration-700" style={{ background: 'var(--coral)', width: `${pct}%` }} />
                </div>
              </div>
            )
          })}
