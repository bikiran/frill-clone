...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    await logJobRun({ job: 'demo-seed', startedAt, durationMs: Date.now() - t0, status: 'success', detail: result })
    return NextResponse.json({ ok: true, ...result })
  } catch (e: any) {
    await logJobRun({ job: 'demo-seed', startedAt, durationMs: Date.now() - t0, status: 'error', error: e?.message })
