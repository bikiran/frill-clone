...
export default function HomePage() {
  ...
  const handleVote = async (ideaId: string) => {
    ...
    // Wait for trigger to complete, then refetch the specific idea's vote count
  ...
  const fetchTopics = async () => {
    const { data } = await supabase.from('ideas').select('topics')
    if (data) {
