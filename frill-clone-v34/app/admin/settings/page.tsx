...
export default function SettingsPage() {
  ...
  const buildSettingsData = (overrides: Record<string, any> = {}) => ({
  ...
  const persistSettings = async (settingsData: Record<string, any>) => {
    try {
  ...
  const uploadFile = async (file: File, folder: string, setter: (url: string) => void) => {
    try {
      const fileName = `${folder}/${Date.now()}-${file.name.replace(/[^a-zA-Z0-9.]/g, '_')}`
      let bucket = 'settings'
      let upload = await supabase.storage.from(bucket).upload(fileName, file, { upsert: true })
      if (upload.error) {
        bucket = 'idea-images'
        upload = await supabase.storage.from(bucket).upload(`settings/${fileName}`, file, { upsert: true })
      }
      if (upload.error) throw upload.error
      const { data: { publicUrl } } = supabase.storage.from(bucket).getPublicUrl(upload.data.path)
      setter(publicUrl)
      const key = folder === 'og' ? 'ogImageUrl' : `${folder}Url`
      await persistSettings(buildSettingsData({ [key]: publicUrl }))
      setSaved(true)
      setTimeout(() => setSaved(false), 2000)
    } catch (err: any) {
  ...
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) uploadFile(file, 'logo', setLogoUrl)
  }

  const handleFaviconUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (file) uploadFile(file, 'favicon', setFaviconUrl)
  }

  const handleOgUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (file) uploadFile(file, 'og', setOgImageUrl)
  }
