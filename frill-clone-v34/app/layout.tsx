...
export default function RootLayout({
  ...
  useEffect(() => {
    const applySettings = (s: any) => {
      if (s) {
        ...
        // Apply theme mode
        if (s.themeMode === 'dark' && typeof document !== 'undefined') {
          document.documentElement.style.setProperty('--canvas', '#1a1a1a')
  ...
  const syncAuthState = (u: any) => {
    setUser(u || null)
    setIsAdmin(u?.email === 'bishalstha76@gmail.com')
  }

  // Keep header in sync with sign-in/sign-out/session expiry without hard refresh
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => syncAuthState(data.session?.user))
            ...
            <div className="flex items-center gap-3">
              {user ? (
                      ...
                      <>
                        ...
                        <div className="absolute top-full right-0 mt-2 w-56 bg-white rounded-lg shadow-2xl border z-40 animate-fade-in-up" style={{ borderColor: 'var(--border)' }}>
                          ...
                          <div className="py-2">
                            ...
                            <Link href="/signup?mode=add" onClick={() => setShowUserMenu(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-gray-50 transition-smooth" style={{ color: 'var(--ink)' }}>
                ...
                <>
                  ...
                  <Link
                    href="/signup"
                    className="px-5 py-2.5 rounded-lg text-sm font-semibold text-white transition-smooth press-effect hover-glow cursor-pointer"
                    style={{ background: 'var(--coral)' }}>
                    Get started
                  </Link>
                </>
