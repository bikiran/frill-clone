...
export default function RoadmapPage() {
  ...
  const persistDropOrder = async (status: string, insertIndex: number) => {
    if (!draggedIdea) return
    const columnItems = ideas
      .filter(i => i.status === status && i.id !== draggedIdea.id)
      .sort((a, b) => (a.order_index ?? 0) - (b.order_index ?? 0) || new Date(a.created_at).getTime() - new Date(b.created_at).getTime())
    const boundedIndex = Math.max(0, Math.min(insertIndex, columnItems.length))
    const nextItems = [...columnItems]
    nextItems.splice(boundedIndex, 0, { ...draggedIdea, status })

    // Re-number the whole destination column so the card lands exactly where it was dropped.
    const updates = nextItems.map((item, index) =>
      supabase.from('ideas').update({ status, order_index: (index + 1) * 100 }).eq('id', item.id)
    )
  ...
  const handleDrop = async (e: React.DragEvent, status: string) => {
    ...
    e.preventDefault()
    if (draggedIdea) {
      const fallbackIndex = ideas.filter(i => i.status === status && i.id !== draggedIdea.id).length
      const insertIndex = dropIndicator?.columnKey === status ? dropIndicator.index : fallbackIndex
      await persistDropOrder(status, insertIndex)
      setDraggedIdea(null)
      setDragOverColumn(null)
      setDropIndicator(null)
    }
          ...
          <div className="relative">
            ...
            {showSortDropdown && (
              <>
                ...
                <div className="absolute top-full right-0 mt-2 w-52 rounded-lg shadow-2xl z-40 overflow-hidden animate-fade-in-up" style={{ background: '#fff', border: '1px solid var(--border)' }}>
                  {SORT_OPTIONS.map(opt => (
                    <button
                      key={opt.key}
                      onClick={() => { setSortBy(opt.key); setShowSortDropdown(false) }}
                      className="w-full px-4 py-2.5 text-left text-sm transition-smooth flex items-center gap-2 cursor-pointer"
                      style={{ color: sortBy === opt.key ? 'var(--coral)' : 'var(--ink)', background: sortBy === opt.key ? 'var(--peach)' : 'transparent' }}>
                      {sortBy === opt.key && '✓ '}
                      {opt.label}
                    </button>
                  ))}
