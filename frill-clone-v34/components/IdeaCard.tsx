    ...
    <div onClick={onClick} className="bg-white rounded-2xl border p-4 sm:p-5 flex gap-3 sm:gap-4 hover:shadow-md hover-lift transition-smooth"
      ...
      <div className="flex-1 min-w-0">
        {idea.cover_image_url && (
          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); setFullImage(idea.cover_image_url) }}
            className="mb-3 block w-full overflow-hidden rounded-xl border bg-gray-50 cursor-zoom-in"
            style={{ borderColor: 'var(--border)' }}
            title="Open image full screen">
            <img src={idea.cover_image_url} alt="Idea image" className="w-full max-h-56 object-cover hover:scale-[1.02] transition-transform duration-300" />
          </button>
        )}
      ...
      {fullImage && (
        <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-4 cursor-zoom-out" onClick={(e) => { e.stopPropagation(); setFullImage(null) }}>
          ...
          <img src={fullImage} alt="Full size idea image" className="max-w-full max-h-full object-contain rounded-xl shadow-2xl" />
        </div>
      )}
    </div>
  )
