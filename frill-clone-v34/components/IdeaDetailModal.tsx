...
export default function IdeaDetailModal({ idea, onClose }: { idea: any; onClose: () => void }) {
  ...
  return (
    <>
      {fullImage && (
        <div className="fixed inset-0 z-[80] bg-black/90 flex items-center justify-center p-4 cursor-zoom-out" onClick={() => setFullImage(null)}>
          <button className="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/10 text-white text-2xl" aria-label="Close image">×</button>
          <img src={fullImage} alt="Full size card image" className="max-w-full max-h-full object-contain rounded-xl shadow-2xl" />
        </div>
      )}
                ...
                <div>
                  ...
                  {coverImageUrl && (
                    <div className="mt-3 relative">
                      <button type="button" onClick={() => setFullImage(coverImageUrl)} className="block w-full cursor-zoom-in"><img src={coverImageUrl} alt="Cover" className="w-full h-24 object-cover rounded-lg" /></button>
                      <button
        ...
        <div className="flex-1 overflow-y-auto bg-white">
          ...
          {coverImageUrl && (
            <div className="px-6 md:px-10 mb-6">
              <button type="button" onClick={() => setFullImage(coverImageUrl)} className="block w-full cursor-zoom-in"><img src={coverImageUrl} alt="Cover" className="w-full h-48 object-cover rounded-xl" /></button>
            </div>
          )}
                  ...
                  {visibleComments.map(comment => (
                    <div key={comment.id} className="p-4 rounded-lg border" style={{ 
                      ...
                      {comment.content.includes('<img') ? (
                        <div className="text-sm prose max-w-none mb-2" style={{ color: 'var(--ink)' }} dangerouslySetInnerHTML={{ __html: comment.content }} />
                      ) : (
